...
def fetch_calls(api_id,api_token,from_ts,to_ts):
    ...
    while True:
        ...
        try:
            r=requests.get("https://api.aircall.io/v1/calls",auth=auth,params=params,timeout=15)
            r.raise_for_status()
        ...
        bar.progress(min(page/20,0.95),text=f"Fetching page {page}… ({len(calls)} calls)")
        if not data.get("meta",{}).get("next_page_link") or page>=40: break
        page+=1
...
CREDS_FILE = os.path.join(os.path.expanduser("~"), ".aircall_dashboard_creds.json")
    ...
    try:
        if os.path.exists(CREDS_FILE):
            with open(CREDS_FILE) as f:
                data = json.load(f)
                return data.get("api_id",""), data.get("api_token","")
    except: pass
    return "", ""

def save_creds(api_id, api_token):
    try:
        with open(CREDS_FILE, "w") as f:
            json.dump({"api_id": api_id, "api_token": api_token}, f)
    except: pass

saved_api_id, saved_api_token = load_saved_creds()
...
logo_col, title_col, spacer, range_col, agent_col, cred_col1, cred_col2, btn_col = st.columns(
    [0.4, 1.2, 0.2, 1.6, 1.8, 1.4, 1.4, 0.7]
)
...
with title_col:
    st.markdown('<div style="margin-top:2px"><div style="font-size:17px;font-weight:700;color:#e2e8f0;line-height:1.2">Aircall Analytics</div><div id="header-sub" style="font-size:11px;color:#4d6280">Dashboard</div></div>', unsafe_allow_html=True)
...
if range_option=="Custom Range":
    cr1,cr2=range_col.columns(2) if False else (range_col,range_col)
    # Show date inputs below range selector when custom
...
with cred_col1:
    api_id=st.text_input("API ID",value=saved_api_id,placeholder="app_xxxx…",key="api_id")

with cred_col2:
    api_token=st.text_input("API Token",value=saved_api_token,placeholder="Token…",type="password",key="api_token")
...
if not api_id or not api_token:
    ...
    <div style="display:flex;align-items:center;justify-content:center;min-height:60vh;flex-direction:column;gap:12px">
        ...
        <div style="color:#e2e8f0;font-size:20px;font-weight:700">Welcome to Aircall Analytics</div>
        <div style="color:#4d6280;font-size:14px">Enter your API ID and API Token above, then click ↻ Update</div>
        <div style="background:#0f1724;border:1px solid #1a2535;border-radius:12px;padding:20px 28px;margin-top:8px;font-size:13px;color:#4d6280;line-height:2">
            <b style="color:#e2e8f0">Where to find your API keys:</b><br>
            1. Log in to Aircall<br>
            2. Go to <b style="color:#00c8f8">Settings → Integrations → API Keys</b><br>
            3. Copy your <b style="color:#00c8f8">API ID</b> and <b style="color:#00c8f8">API Token</b>
        </div>
...
if update_btn:
    ...
    if api_id and api_token:
        save_creds(api_id, api_token)

if "df" not in st.session_state or update_btn:
...
ans_rate=pct(len(answered),total)
...
for col,args in zip(st.columns(8),[
    ...
    ("Answer Rate",f"{ans_rate}%",f"{len(answered)} answered","#22c55e"),
...
missed_nums=set(missed["raw_digits"].dropna())
...
with tab2:
    ...
    def color_rate(v):
        ...
        return ""
    st.dataframe(tbl.style.applymap(color_rate,subset=["Answer Rate %"]),use_container_width=True,hide_index=True)
...
with tab3:
    ...
    def style_log(d):
        ...
        sc={"done":"color:#22c55e","missed":"color:#f43f5e","voicemail":"color:#f59e0b"}
        dc={"inbound":"color:#00c8f8","outbound":"color:#8b5cf6"}
        for i in d.index:
            s.loc[i,"Status"]=sc.get(log.loc[i,"status"],"")
            s.loc[i,"Direction"]=dc.get(log.loc[i,"direction"],"")
        return s

    st.dataframe(dl.style.apply(style_log,axis=None),use_container_width=True,hide_index=True,height=500)
