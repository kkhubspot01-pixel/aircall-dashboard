...
if range_option=="Custom Range":
    ...
    to_dt=datetime(now.year,now.month,now.day,23,59,59,tzinfo=timezone.utc)  # end of today
...
.kpi-card{{background:{CARD};border:1px solid {BORDER};border-radius:12px;padding:14px 16px;position:relative;overflow:hidden;min-height:100px;}}
.kpi-card-top{{height:2px;position:absolute;top:0;left:0;right:0;}}
.kpi-label{{color:{MUTED};font-size:9px;text-transform:uppercase;letter-spacing:1px;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
.kpi-value{{font-size:26px;font-weight:700;font-family:'DM Mono',monospace;margin:4px 0 2px;line-height:1.1;white-space:nowrap;}}
.kpi-sub{{color:{MUTED};font-size:10px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}}
.insight-card{{background:{CARD};border:1px solid {BORDER};border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:14px;min-height:90px;}}
...
if "df" not in st.session_state or st.session_state.get("range_key")!=range_key:
    ...
    st.session_state["range_label"]=f"{from_dt.strftime('%d %b %Y')} – {(to_dt - timedelta(seconds=1)).strftime('%d %b %Y')} (UTC)"
...
with st.expander("🔍 Status Breakdown (click to verify vs Aircall)"):
    if not df_all.empty:
        bd=df_all.groupby(["direction","status"]).size().reset_index(name="count").sort_values("count",ascending=False)
        st.dataframe(bd,hide_index=True,use_container_width=True)
        st.markdown(f"**Raw API total: {len(df_all)}** | missed_call_reason unique values:")
        if "missed_call_reason" in df_all.columns:
            st.write(df_all["missed_call_reason"].value_counts().to_dict())
...
for col,args in zip(st.columns(9),[
    ...
    ("Inbound Ans.%",f"{inbound_ans_rate}%",f"{len(inbound_answered)}/{inbound_total} inbound","#22c55e"),
    ("Outbound Pickup%",f"{outbound_pickup_rate}%",f"{len(outbound_connected)}/{outbound_total} outbound","#22c55e"),
    ("Missed",len(inbound_missed)+len(outbound[outbound["connected"]==False]),f"{len(inbound_missed)} in · {len(outbound[outbound['connected']==False])} out","#f43f5e"),
