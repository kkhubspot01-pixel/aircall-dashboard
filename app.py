...
# Theme colors — set after dark_mode toggle is read (see below)
# Defaults used only for helper functions before toggle renders
ACCENT = "#00c8f8"
...
# ── Dynamic CSS (injected after theme is known) ───────────────────────────────
st.markdown(f"""
<style>
...
.section-header{{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:2px;
    color:{TEXT};padding:4px 0 4px 12px;margin:16px 0 10px;border-left:3px solid #00c8f8;}}
.kpi-card{{background:{CARD};border:1px solid {BORDER};border-radius:12px;padding:16px 18px;position:relative;overflow:hidden;}}
...
.insight-card{{background:{CARD};border:1px solid {BORDER};border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:14px;}}
.insight-icon-wrap{{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;}}
