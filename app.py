...
# ── Dynamic theme ─────────────────────────────────────────────────────────────
# dark_mode defined above from toggle
BG       = "#070d1a" if dark_mode else "#f0f4f8"
CARD     = "#0f1724" if dark_mode else "#ffffff"
BORDER   = "#1a2535" if dark_mode else "#e2e8f0"
TEXT     = "#e2e8f0" if dark_mode else "#0f172a"
MUTED    = "#4d6280" if dark_mode else "#64748b"
ACCENT   = "#00c8f8"
INPUT_BG = "#070d1a" if dark_mode else "#f8fafc"
...
.stSelectbox>div>div,.stMultiSelect>div>div,.stTextInput>div>div>input,.stDateInput>div>div>input{{
    background:{INPUT_BG} !important;border-color:{BORDER} !important;color:{TEXT} !important;font-size:12px !important;}}
...
/* Tabs */
.stTabs [data-baseweb="tab-list"]{{background:{CARD};border-radius:8px;border:1px solid {BORDER};gap:2px;padding:4px;}}
...
.stButton>button{{background:{CARD} !important;color:{TEXT} !important;font-weight:600 !important;
    border:1px solid {BORDER} !important;border-radius:8px !important;font-size:12px !important;padding:6px 14px !important;}}
...
div[data-testid="stFormSubmitButton"]>button,button[kind="primary"]{{
    background:{ACCENT} !important;color:#070d1a !important;border:none !important;font-weight:700 !important;}}

/* Toggle */
.stToggle label{{color:{MUTED} !important;font-size:11px !important;}}
...
.section-header{{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:2px;
    color:{TEXT};padding:4px 0 4px 12px;margin:16px 0 10px;border-left:3px solid {ACCENT};}}

/* KPI card */
.kpi-card{{background:{CARD};border:1px solid {BORDER};border-radius:12px;
    padding:16px 18px;position:relative;overflow:hidden;}}
.kpi-card-top{{height:2px;position:absolute;top:0;left:0;right:0;}}
.kpi-label{{color:{MUTED};font-size:10px;text-transform:uppercase;letter-spacing:1.2px;margin-bottom:4px;}}
.kpi-value{{font-size:28px;font-weight:700;font-family:'DM Mono',monospace;margin:4px 0 2px;line-height:1.1;}}
.kpi-sub{{color:{MUTED};font-size:11px;}}

/* Insight card */
.insight-card{{background:{CARD};border:1px solid {BORDER};border-radius:12px;
    padding:14px 16px;display:flex;align-items:center;gap:14px;}}
.insight-icon-wrap{{width:40px;height:40px;border-radius:10px;display:flex;
    align-items:center;justify-content:center;font-size:18px;flex-shrink:0;}}
.insight-label{{color:{MUTED};font-size:10px;text-transform:uppercase;letter-spacing:1px;}}
.insight-value{{font-size:20px;font-weight:700;font-family:'DM Mono',monospace;margin:2px 0;line-height:1.1;}}
.insight-sub{{color:{MUTED};font-size:11px;}}
...
# PT defined later after dark_mode toggle is read
# placeholder — will be set after theme toggle
...
def quarter_bounds(offset=0):
    ...
    q=(now_utc.month-1)//3-offset
    year=now_utc.year
    while q<0: q+=4;year-=1
    start=datetime(year,q*3+1,1,0,0,0,tzinfo=timezone.utc)
    end_month=q*3+3
    end_year=year if end_month<=12 else year+1
    end_month=end_month if end_month<=12 else end_month-12
    end_day=calendar.monthrange(end_year,end_month)[1]
    return start,datetime(end_year,end_month,end_day,23,59,59,tzinfo=timezone.utc)
...
for c in calls:
    ...
    answered = c.get("answered_at")   # null if missed/unanswered
    ended    = c.get("ended_at")
    direction= c.get("direction","")
    status   = c.get("status","")

    # Real talk time = ended_at - answered_at (excludes ring time)
    # Aircall docs: use ended_at - answered_at for actual talking time
    if answered and ended:
        talk_time = max(0, ended - answered)
    else:
        talk_time = 0

    # duration from API includes ringing — use talk_time for avg duration display
    raw_duration = c.get("duration") or 0

    # Was the call actually picked up?
    # Inbound: status=="done" means answered
    # Outbound: answered_at is not null means other side picked up
    ...
    rows.append({
        ...
        "direction":   direction,
        "status":      status,
        "duration":    raw_duration,       # raw (includes ring)
        "talk_time":   talk_time,          # real talk time
        "connected":   connected,          # True if other side picked up
        "answered_at": answered,
        "waiting_duration": c.get("waiting_duration") or 0,
...
# ═══════════════════════════════════════════════════════
now = datetime.now(timezone.utc)  # always UTC to match Aircall

logo_col, title_col, spacer, range_col, agent_col, theme_col, btn_col = st.columns(
    [0.4, 1.4, 0.3, 1.8, 2.0, 0.5, 0.7]
)
...
# Compute from/to based on range — all in UTC to match Aircall exactly
def utc_day_start(d):
    """UTC midnight for a given date"""
    return datetime(d.year, d.month, d.day, 0, 0, 0, tzinfo=timezone.utc)

def utc_day_end(d):
    """UTC 23:59:59 for a given date"""
...
if range_option=="Custom Range":
    ...
    from_dt=utc_day_start(from_date)
    to_dt=utc_day_end(to_date)
    ...
    from_dt=utc_day_start((now-timedelta(days=days)).date())
    to_dt=utc_day_end(now.date())  # end of today in UTC (23:59:59)
...
with theme_col:
    ...
    dark_mode=st.toggle("🌙",value=True,help="Toggle dark/light mode")

with btn_col:
    st.markdown('<div style="height:22px"></div>',unsafe_allow_html=True)
    update_btn=st.button("↻ Update",use_container_width=True)

# Plotly theme — set after dark_mode is known
PT=dict(
...
if "df" not in st.session_state or st.session_state.get("range_key")!=range_key:
    ...
    st.session_state["range_label"]=f"{from_dt.strftime('%d %b %Y')} – {to_dt.strftime('%d %b %Y')} (UTC)"
...
# Answered = connected (answered_at is not null)
answered  = df[df["connected"]==True]
missed    = df[(df["status"]=="missed") | ((df["direction"]=="inbound")&(df["connected"]==False)&(df["status"]!="done"))]
voicemail = df[df["status"]=="voicemail"]

# Inbound Answer Rate = inbound calls where answered_at is not null / total inbound
inbound_answered = inbound[inbound["connected"]==True]
inbound_missed   = inbound[inbound["connected"]==False]
inbound_total    = len(inbound)
inbound_ans_rate = pct(len(inbound_answered),inbound_total) if inbound_total>0 else 0

# Outbound Pickup Rate = outbound where other side picked up (answered_at not null) / total outbound
outbound_connected   = outbound[outbound["connected"]==True]
outbound_total       = len(outbound)
outbound_pickup_rate = pct(len(outbound_connected),outbound_total) if outbound_total>0 else 0

# Use talk_time (ended_at - answered_at) for avg duration — excludes ring time
# Only count connected calls for avg duration
connected_calls = df[df["talk_time"]>0]
avg_dur   = int(connected_calls["talk_time"].mean()) if len(connected_calls)>0 else 0
total_dur = int(df["talk_time"].sum())
...
with st.expander("🔍 Debug — Compare with Aircall Dashboard"):
    import datetime as _dt
    rk=st.session_state.get("range_key","")
    if rk:
        p=rk.split("_")
        f_utc=_dt.datetime.fromtimestamp(int(p[0]),tz=_dt.timezone.utc)
        t_utc=_dt.datetime.fromtimestamp(int(p[1]),tz=_dt.timezone.utc)
        st.code(f"API from: {int(p[0])} = {f_utc.strftime('%Y-%m-%d %H:%M:%S UTC')}\nAPI to:   {int(p[1])} = {t_utc.strftime('%Y-%m-%d %H:%M:%S UTC')}")
    st.markdown("**All status + direction combinations:**")
    if not df_all.empty:
        breakdown=df_all.groupby(["direction","status"]).size().reset_index(name="count")
        breakdown["pct"]=( breakdown["count"]/len(df_all)*100).round(1).astype(str)+"%"
        st.dataframe(breakdown,hide_index=True,use_container_width=True)
    st.markdown("**Calls per day (UTC):**")
    if not df_all.empty:
        daily=df_all.groupby(["date","direction"]).size().reset_index(name="count")
        daily_pivot=daily.pivot(index="date",columns="direction",values="count").fillna(0).reset_index()
        daily_pivot["total"]=daily_pivot.drop("date",axis=1).sum(axis=1)
        st.dataframe(daily_pivot,hide_index=True,use_container_width=True)
        st.markdown(f"**Grand total: {len(df_all)} calls** | First call: `{df_all['started_at'].min()}` | Last call: `{df_all['started_at'].max()}`")
    st.markdown("**Sample of raw call data (first 5):**")
    if not df_all.empty:
        st.dataframe(df_all[["started_at","direction","status","duration","agent_name"]].head(5),hide_index=True,use_container_width=True)


# ── KPI Row ───────────────────────────────────────────
st.markdown('<div class="section-header">Key Metrics</div>',unsafe_allow_html=True)
for col,args in zip(st.columns(9),[
    ("Total Calls",total,f"{cpd} calls/day","#00c8f8"),
    ("Inbound Ans. Rate",f"{inbound_ans_rate}%",f"{len(inbound_answered)}/{inbound_total} inbound","#22c55e"),
    ("Outbound Pickup",f"{outbound_pickup_rate}%",f"{len(outbound_connected)}/{outbound_total} outbound","#22c55e"),
    ("Missed",len(inbound_missed)+len(outbound[outbound["connected"]==False]),f"In:{len(inbound_missed)} Out:{len(outbound[outbound['connected']==False])}","#f43f5e"),
...
with tab2:
    ag=df.groupby("agent_name").agg(
        ...
        total_dur=("talk_time","sum"),
        avg_dur=("talk_time",lambda x:x[x>0].mean() if (x>0).any() else 0),
    ).reset_index()
