...
def build_df(calls):
    ...
    rows=[{"id":c.get("id"),"direction":c.get("direction",""),"status":c.get("status",""),
        ...
        "started_at":datetime.fromtimestamp(c["started_at"],tz=timezone.utc).replace(tzinfo=None) if c.get("started_at") else None,
...
# Inbound Answer Rate = inbound calls answered / total inbound
...
# Outbound Pickup Rate = outbound calls picked up (done + duration > 0) / total outbound
outbound_connected=df[(df["direction"]=="outbound")&(df["status"]=="done")&(df["duration"]>0)]
...
# ── Debug expander ────────────────────────────────────
with st.expander("🔍 Debug — API raw data (use to verify against Aircall dashboard)"):
    st.markdown("**Timestamps sent to Aircall API:**")
    rk = st.session_state.get("range_key","")
    if rk:
        parts = rk.split("_")
        if len(parts)==2:
            import datetime as _dt
            f_utc = _dt.datetime.fromtimestamp(int(parts[0]),tz=_dt.timezone.utc)
            t_utc = _dt.datetime.fromtimestamp(int(parts[1]),tz=_dt.timezone.utc)
            st.code(f"from: {int(parts[0])} = {f_utc.strftime('%Y-%m-%d %H:%M:%S UTC')}\nto:   {int(parts[1])} = {t_utc.strftime('%Y-%m-%d %H:%M:%S UTC')}")
    c1,c2,c3 = st.columns(3)
    c1.metric("Raw API calls fetched", len(df_all))
    c2.metric("After agent filter", len(df))
    c3.metric("Unique dates in data", df_all["date"].nunique() if not df_all.empty else 0)
    st.markdown("**Status breakdown (raw from API):**")
    if not df_all.empty:
        status_counts = df_all.groupby(["direction","status"]).size().reset_index(name="count")
        st.dataframe(status_counts, hide_index=True, use_container_width=True)
    st.markdown("**Calls per date (UTC):**")
    if not df_all.empty:
        daily = df_all.groupby("date").size().reset_index(name="calls")
